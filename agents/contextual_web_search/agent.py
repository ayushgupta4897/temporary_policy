...
TIMEOUT_READ = 10800.0  # 3 hours for extended search operations
TIMEOUT_WRITE = 120.0
TIMEOUT_POOL = 60.0
...
# Output directory
OUTPUT_DIR = "contextual_search_outputs"

# Display configuration
TOP_CITATIONS_DISPLAY = 8
TITLE_TRUNCATE_LENGTH = 80
TOTAL_SOURCE_TIERS = 30
...
class SimpleWebSearch:
    ...
    def __init__(self, batch_size: int = DEFAULT_BATCH_SIZE):
        # Use extended timeout for long contextual search operations
        timeout_config = {
            'connect': TIMEOUT_CONNECT,
            'read': TIMEOUT_READ,  # 3 hours for extended search operations
            'write': TIMEOUT_WRITE,
            'pool': TIMEOUT_POOL
        }
    ...
    def _merge_results(self, all_results: List[Dict]) -> List[Dict]:
        ...
        unique_results.sort(key=lambda x: x.get('year', DEFAULT_YEAR_SORT), reverse=True)
...
def main():
    ...
    searcher = SimpleWebSearch(batch_size=DEFAULT_BATCH_SIZE)
