...
interface UnifiedChatProps {
  queryId: string
  agentType: 'policy' | 'system-compass' | 'dsm' | 'impact-analysis'
  title: string
  description: string
  iconColor?: string
}
...
export default function UnifiedChat({
  ...
  // Build API paths based on agent type
  const getApiPath = () => {
    if (agentType === 'policy') {
      return `/api/queries/${queryId}/chat`
    }
    return `/api/${agentType}/${queryId}/chat`
  }
  ...
  useEffect(() => {
    const storageKey = `${agentType}_chat_session_${queryId}`
  ...
  if (isChatAvailable === false) {
    return (
      <div className="flex flex-col items-center justify-center h-96 text-neutral-400">
        <ChatUnavailableIcon />
        <p className="mt-4 text-sm">Chat is not available for this analysis yet</p>
        <p className="text-xs text-neutral-500 mt-2">Please wait for analysis to complete</p>
      </div>
    )
  }
    ...
    <div className="flex flex-col h-full bg-dark-500/30 border border-dark-300/40 rounded-lg overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-dark-300/40 bg-dark-400/20">
        <div className="flex items-center gap-3">
          <ChatIcon className={`text-${iconColor}-500`} />
          <div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {messages.length === 0 ? (
          <EmptyState title={title} iconColor={iconColor} />
        ) : (
      ...
      <div className="p-4 border-t border-dark-300/40 bg-dark-400/10">
        <div className="flex gap-3">
          <textarea
            ...
            placeholder={`Ask a question about this ${agentType === 'policy' ? 'policy' : 'analysis'}...`}
            disabled={isLoading}
            rows={1}
        ...
        <div className={`px-4 py-3 rounded-2xl ${
          ...
          {isUser ? (
            ...
            <div className="text-sm text-neutral-200 prose prose-invert prose-sm max-w-none
                          ...
                          prose-table:text-sm prose-th:text-xs prose-td:text-xs">
...
// Empty State
function EmptyState({ title, iconColor }: { title: string, iconColor: string }) {
  return (
    <div className="flex flex-col items-center justify-center h-full text-center py-12">
      <div className={`w-16 h-16 rounded-full bg-${iconColor}-600/10 flex items-center justify-center mb-4`}>
        <ChatIcon className={`w-8 h-8 text-${iconColor}-500`} />
      </div>
      <h4 className="text-neutral-200 font-medium mb-2">Start a Conversation</h4>
      <p className="text-neutral-400 text-sm max-w-sm">
        Ask questions about the {title.toLowerCase()} and get instant answers based on the analysis
      </p>
    </div>
  )
}
