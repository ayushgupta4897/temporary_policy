...
interface FloatingSystemCompassChatProps {
  queryId: string
}

const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8000'

export default function FloatingSystemCompassChat({ queryId }: FloatingSystemCompassChatProps) {
  ...
  const apiBasePath = `/api/system-compass/${queryId}/chat`

  // Initialize session on mount
  useEffect(() => {
    const storageKey = `system_compass_chat_session_${queryId}`
  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          onClick={toggleChat}
          className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-blue-600 hover:bg-blue-500
                   text-white rounded-full shadow-lg hover:shadow-xl
                   flex items-center justify-center transition-all duration-300
                   hover:scale-110 active:scale-95"
          aria-label="Open Systems Chat"
        ...
        <div className={`fixed bottom-6 right-6 z-50 w-96 bg-dark-500 border border-dark-300/60
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-dark-300/40
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-neutral-200">Systems Thinking</span>
            </div>
            <div className="flex items-center gap-2">
              <button
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 h-[480px]">
                {messages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center">
                    ...
                    <p className="text-neutral-300 text-sm font-medium mb-1">Systems Thinking</p>
                    <p className="text-neutral-500 text-xs">Ask about the evidence graph</p>
                  </div>
                ) : (
                  <>
...
// Message Bubble Component (simplified version)
      ...
      <div className={`max-w-[85%] px-3 py-2 rounded-2xl text-sm ${
        ...
        {isUser ? (
          ...
          <div className="prose prose-invert max-w-none text-sm prose-p:my-1 prose-headings:text-sm">
            ...
            {message.isStreaming && <span className="inline-block w-1 h-3 bg-blue-500 ml-0.5 animate-pulse" />}
          </div>
        )}
      </div>
    </div>
