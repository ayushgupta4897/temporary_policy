...
"""
    ...
    SEGB_CITATIONS_ONLY_PROMPT = """# 📚 High-Yield Citation Extraction

**Role**: Senior evidence analyst conducting exhaustive web search for maximum citation extraction.  
**Task**: Search specified high-trust sources and extract **as many relevant citations as possible** (target: 8-12 citations per source type).
...
1. **Define the DAG hypothesis**: Build a DENSE, comprehensive systems map with **minimum 20-25 nodes** from the 40 issues listed above. **CRITICAL: Every node MUST be one of the 40 specific issues. Do NOT create new topics.** Map each node to its exact category and issue name. Include extensive **feedback loops** and **cross-category connections**. 
   - **Density Target**: Aim for edge-to-node ratio of 1.2-1.5 (e.g., 20 nodes → 24-30 edges)
   - **Cross-Category Links**: Ensure nodes from different categories connect (e.g., Financial → Mental Health → Social)
...
2. **Source discovery (geography-constrained trust ladder)**: 
   - **TIER 1**: {geography} government portals, national statistical bureaus, local health authorities
   - **TIER 2**: GCC/Gulf regional organizations, comparative studies including {geography}
   - **TIER 3**: International orgs (UN, World Bank, WHO, OECD) BUT only if they have {geography}-specific data
   - **TIER 4**: Peer-reviewed journals (PNAS, Nature Human Behaviour) BUT only if studying {geography} populations
   - **FORBIDDEN**: Generic global studies without {geography} context or comparative data
...
5. **GEOGRAPHY COMPLIANCE CHECK**: Before including any node in your graph:
   ✅ **REQUIRED**: Find {geography}-specific data, GCC comparative data, or Arab regional context
   ❌ **FORBIDDEN**: Global-only statistics without {geography} relevance
   🔍 **TEST**: Can you answer "What does this look like specifically in {geography}?" with data?
   
6. **Local relevance**: If local ({geography}) evidence exists, weight it higher. If not, use nearest-neighbor contexts (GCC/MENA) with explicit geographic context tagging.
...
8. **Normalization & weighting**: Compute per-edge weights in [0,1] using the scoring below; normalize so outgoing weights from each driver sum to ≤1.
...
* Outgoing weights from a driver **should not exceed 1.0** after normalization.
* Include **feedback edges** where warranted (e.g., mental health issues ↔ substance use).
* Keep total nodes **15–35** for readability from the available 40 issues.
...
1. **GEOGRAPHY FIRST**: Every node must have {geography}-relevant evidence. Node inclusion hierarchy:
   - **TIER 1**: Direct {geography} government data, local institution reports
   - **TIER 2**: GCC/Gulf regional comparative data including {geography}
   - **TIER 3**: Arab/MENA regional data with {geography} context
   - **FORBIDDEN**: Generic global nodes without {geography} relevance

2. **DENSE SYSTEMS MAP**: Build comprehensive map (aim for 20-25 nodes) BUT only include nodes with {geography}-relevant evidence

3. **GEOGRAPHY VALIDATION**: Before including any node, verify:
   - Can you find {geography}-specific data for this node?
   - Is there at least GCC/Gulf regional context?
   - Does the node affect {geography} specifically?
   - If NO → exclude the node or find better evidence

4. **CITATION ENHANCEMENT**: Use extracted citations as supporting evidence for geographically-relevant nodes

5. **REJECT GLOBAL FALLBACKS**: Do not include nodes just because global data exists - only include if {geography}-relevant
...
* **Expected key nodes** (ONLY from the 40 issues):
  - **Drivers**: Cost of living, Gender imbalance in pay & opportunities, Housing prices, Access to quality housing
  - **Mediators**: Mental health issues (depression, anxiety), Substance use (alcohol, drugs), Social support networks, Work-life balance
  - **Status Quo**: Divorce rates, Incidence of family debt
  - **Outcomes/Implications**: Quality family time, Social isolation/loneliness, Community engagement & volunteering
  - **Feedback loops**: Mental health issues ↔ Substance use, Family debt ↔ Cost of living
* **Key Disruptors affecting edges**:
  - "Global economic decline" amplifies: Cost of living → Family debt
