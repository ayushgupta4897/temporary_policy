"""
Graph Repository - DUMB CRUD operations only
...
class GraphRepository:
    """Repository for graph query data persistence."""
    ...
    def create_query(self, query_id: str, query_text: str, display_title: str,
                    analysis_mode: str = "graph", metadata: Dict = None) -> bool:
        """Create a new graph query record."""
    ...
    def list_queries(self, query_type: str = "graph") -> List[Dict]:
    ...
    def update_query_progress(self, query_id: str, progress: int, current_step: str) -> bool:
        """Update query progress (legacy method for backward compatibility)."""
        return self.table_client.update_query_progress(query_id, progress, current_step)

    def update_query_steps(self, query_id: str, steps: List[Dict]) -> bool:
        """Update query with step-based progress."""
        return self.table_client.update_query_steps(query_id, steps)
    ...
    def upload_report(self, query_id: str, filename: str, content: str) -> str:
        """Upload a graph output to blob storage."""
